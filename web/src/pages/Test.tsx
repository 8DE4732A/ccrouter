...
function parseNonStreamBlocks(fmt: ApiFormat, obj: unknown, rawText: string): OutputBlock[] {
  ...
  if (fmt === 'openai-responses') {
    const output = (o.output as {type?: string; content?: {type?: string; text?: string}[]}[]) ?? []
    const blocks: OutputBlock[] = []
    for (const item of output) {
      if (item.type === 'reasoning') {
        // skip — no visible thinking content in responses format
        continue
      }
      if (item.type === 'message') {
        for (const c of item.content ?? []) {
          if (c.type === 'output_text' && c.text) blocks.push({ type: 'text', content: c.text })
        }
      }
    }
    return blocks.length > 0 ? blocks : [{ type: 'text', content: rawText }]
          ...
          <div className="card">
            <div className="card-body-simple">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {(fmt === 'openai' || fmt === 'anthropic' || fmt === 'openai-responses') && (
                  <div>
                    <div className="field-label" style={{ marginBottom: 5 }}>
                      思考等级
                      <span className="dim" style={{ marginLeft: 6 }}>
                        {fmt === 'anthropic' ? 'thinking.budget_tokens' : fmt === 'openai-responses' ? 'reasoning.effort' : 'reasoning_effort'}
                      </span>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {(['off', 'low', 'medium', 'high'] as ThinkingLevel[]).map(level => (
                        ...
                        >
                          ...
                          {(fmt === 'openai' || fmt === 'openai-responses') && level !== 'off' && (
                            <span style={{ display: 'block', fontSize: 10, opacity: 0.5 }}>effort</span>
                          )}
                        </button>
                      ))}
                    </div>
